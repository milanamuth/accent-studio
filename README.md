...
## Phoneme scoring with Azure (Phase 2, built in)

The app includes objective per-phoneme scoring via Azure Pronunciation Assessment,
layered on top of the AI coaching. On benchmark days the recording is first scored
by Azure, and those hard numbers are fed into the AI coach so its advice is grounded
in measured data rather than impression alone.

To enable it:
1. Create an Azure account and a **Speech Services** resource on the free **F0** tier.
2. From the resource's **Keys and Endpoint** page, copy **KEY 1** and note the **region** (e.g. `centralindia`).
3. In Vercel → Settings → Environment Variables, add:
   - `AZURE_SPEECH_KEY` = your KEY 1
   - `AZURE_SPEECH_REGION` = your region (e.g. `centralindia`)
4. Redeploy.

If these are not set, the app silently falls back to AI-only coaching, so nothing breaks.

Notes:
- Azure scoring only runs on **benchmark days**, where the reference passage is known.
- The Azure short-audio endpoint caps at ~60 seconds, so benchmark reads should stay under ~55s (the Rainbow Passage is ~35s).
- Free F0 tier covers 5 audio hours per month, far more than daily practice needs.
